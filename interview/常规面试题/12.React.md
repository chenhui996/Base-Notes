...
- **作用**：管理组件 内部状态。
- **核心用途**：用于在 函数组件 中 声明状态变量，并触发重新渲染。
...
- **作用**：用来保存一个 **在组件生命周期中** 不会变的 **可变引用对象**，常用于 **DOM 引用** 和 **在闭包中获取最新变量值**。
- **核心用途**：
  - 在 **跨渲染过程中** **存储数据** 或 **dom 元素引用**，**不触发重新渲染**。（跨渲染：组件每次重新渲染时）
  - 常用于 **访问 dom 元素**、**保存中间状态** 或 **避免闭包问题**。
- **本质**：
