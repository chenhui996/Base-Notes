...
- **问题**：在这个股票行情项目中，你遇到过哪些技术难点？是如何解决的？
- **准备**：描述具体的技术难点，如大数据量渲染、高频推送数据、断线重连机制等。分享解决方案，如手搓虚拟滚动、使用shared-worker处理数据、引入onVisibleChange实现断线重连等。
  - 股票 kline 模块的推送，逻辑较为复杂：
    - 难点一: 推送：
      - 大数据量渲染、高频推送数据、断线重连机制。
      - 不是简单的收到推送后，直接覆盖，而是全部的计算，都放在前端，接口里只有基础数据。
        - 常规行情推送：
          - 揉进最后一条数据，然后直接覆盖。
          - 若是时间点特殊，过了最新一根时间，例如进入了下一分钟的 0秒，那么，就需要前端自行先新增一条数据（继承上一根），然后收到推送后，再揉进最后一条数据。
        - 延迟推送：
          - 当推送因为各类原因延迟：
            - 前端需要自行计算
              - 分钟线：进入新的一分钟，需要画新的柱子。
              - 日线：进入新的一天，需要画新的柱子。
              - 坑：当前端自行补过多数据后，推送又来了，那么，前端需要自行计算纠正：
                - 如：现在是 11点整，延迟了半小时。前端自行补了半小时的数据，然后，推送又来了，那么，前端需要自行计算纠正。
                  - 纠正：
                    - 10:30那一根，需要纠正。
                    - 11:30～11:00的所有根，均需要根据10:30那一根，进行纠正。
    - 难点二: 前端揉数据：
      - 揉数据：1分钟线揉成 -> 5分钟线、15分钟线、30分钟线、60分钟线、日线、周线、月线、年线。
      - 为什么要揉数据：减少数据传输量，提高性能。
      - 如何揉数据：
        - 1分钟线揉成 -> 5分钟线、15分钟线、30分钟线、60分钟线、日线、周线、月线、年线。
          - 揉法：
            - 将 1 分钟线数据按照时间分组，然后计算每组的开盘价、最高价、最低价、收盘价。
        - 为什么要揉数据：减少数据传输量，提高性能。
      - A股由于时间的边界情况多，揉数据时就会比较难受：
        - 开盘时间：
          - 9:30:00 - 9:30:59
        - 收盘时间：
          - 14:59:00 - 15:00:59
    - 难点三: 开着当前页面，又过夜的用户。早上全部用户，瞬间获取昨收，服务器压力太大：
      - 解决方案：
        - 分段获取昨收，减少服务器压力。
          - 怎么分段 -> 根据时间段，分段获取昨收。
    - 难点四: 
